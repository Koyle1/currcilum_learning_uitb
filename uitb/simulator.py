...
class Simulator(gym.Env):
  ...
  def _restore_success_anchor(self, anchor):
    self._data.qpos[:] = anchor["qpos"]
    self._data.qvel[:] = anchor["qvel"]
    self._data.ctrl[:] = anchor["ctrl"]

    if anchor["act"] is not None and self._model.na > 0:
        self._data.act[:] = anchor["act"]

    for attr in ("_motor_act", "_motor_smooth_avg", "_sigdepnoise_acc", "_constantnoise_acc"):
        if attr in anchor and hasattr(self.bm_model, attr):
            setattr(self.bm_model, attr, copy.deepcopy(anchor[attr]))
  ...
  def step(self, action):
    ...
    if 'llc' in self.config:  # if HRL approach is used
      ...
      llc_action, _states = self.llc_model.predict(
        self.get_llcobservation(action), deterministic=True
      )
      ...
      mujoco.mj_step(
        self._model,
        self._data,
        nstep=int(self._run_parameters["frame_skip"])
      )
