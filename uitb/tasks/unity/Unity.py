...
class UnityEnv(BaseTask):
  ...
  def _update(self, model, data):
    ...
    # Track progress through the original long episode.
    # This counter should only be reset on a REAL reset.
    self._hard_reset_step_count += 1

    # Only hard-reset once the current Unity attempt finishes after the step cap has been exceeded.
    max_steps_reached = self._hard_reset_step_count > self._max_steps

    # Send end effector position and rotation to unity, get reward and image from camera
    obs, reward, is_app_finished, log_dict = self._unity_client.step(
        self._create_state(model, data),
        False
    )
    log_dict = json.loads(log_dict)
    self._time = obs["time"]

    if max_steps_reached and is_app_finished:
        terminated = False
        truncated = True
        self._needs_hard_reset = True
        self._info["termination"] = "max_steps_reached"

    elif is_app_finished:
        terminated = True
        truncated = False
        self._needs_hard_reset = False
        self._info["termination"] = "attempt_done"

    else:
        terminated = False
        truncated = False
        self._info.pop("termination", None)
